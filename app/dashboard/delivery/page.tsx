...
import {
  ...
  CheckCircle2, TrendingUp, Flame, Loader2, AlertCircle, ShieldCheck,
  KeyRound, Gift, ChefHat, Calendar,
} from "lucide-react";

// ✅ FIXED: claimedBy accepts both object and string
...
// ✅ Helper to safely extract name from claimedBy
function getClaimedByName(claimedBy: Assignment["claimedBy"], fallback?: string): string {
  if (!claimedBy && !fallback) return "";
  if (typeof claimedBy === "object" && claimedBy !== null) return claimedBy.name || fallback || "";
  if (typeof claimedBy === "string") return claimedBy || fallback || "";
  return fallback || "";
}
...
function OtpInput({ assignment, onDelivered }: { assignment: Assignment; onDelivered: () => void }) {
  ...
  async function handleVerify() {
    if (otp.length !== 6) { setError("Enter a 6-digit OTP."); return; }
    setError(null);
    setLoading(true);
    try {
  ...
  return (
    <div className="mt-3 border-t border-gray-200 pt-3 space-y-2">
      ...
      <div className="flex gap-2 items-center flex-wrap">
        ...
        <button onClick={() => { setOpen(false); setError(null); setOtp(""); }}
          className="text-gray-400 text-xs font-medium hover:text-gray-600 transition-colors">Cancel</button>
      </div>
      <p className="text-[11px] text-gray-400">Ask the NGO representative to share the 6-digit OTP from their phone.</p>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              ...
              <span className="font-extrabold text-blue-600 text-lg tracking-tight">Setu</span>
            </div>
          </Link>
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:flex items-center gap-2 ml-1 pl-3 border-l border-gray-100">
              ...
              <div className="leading-tight">
                ...
                <p className="text-[11px] text-blue-600 font-semibold">Delivery Partner</p>
              </div>
            </div>
          ...
          <div className="relative flex items-center justify-between gap-6 flex-wrap">
            <div>
              ...
              <p className="text-blue-100 text-sm max-w-sm">
                {pending} pickup{pending !== 1 ? "s" : ""} waiting. Verify OTP on delivery.
              </p>
            </div>
            <div className="flex items-center gap-2 bg-white/15 border border-white/20 rounded-2xl px-5 py-3">
              ...
              <div>
                <p className="text-white font-extrabold text-xl leading-tight">{totalDone}</p>
                <p className="text-blue-200 text-xs font-medium">delivered today</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="px-7 py-5 space-y-4">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-sm text-gray-400 max-w-xs">You will see assignments here once an NGO claims a listing.</p>
                </div>
              ) : (
                assignments.map((item) => {
                  // ✅ safely get NGO name
                  const ngoName = getClaimedByName(item.claimedBy, item.claimedByName);
                  return (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-3 flex-wrap">
                            <div>
                              ...
                              {/* ✅ show quantityStr if available, else quantity */}
                              <p className="text-xs text-gray-500 mt-0.5">{item.quantityStr || item.quantity}</p>
                            </div>
                            <span className="flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full bg-orange-100 text-orange-700 border border-orange-200">
                              <span className="w-1.5 h-1.5 rounded-full bg-orange-500" />Pickup Pending
                            </span>
                          </div>
                          <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1">
                            {item.donorName && (
                              <span className="flex items-center gap-1 text-xs text-gray-400">
                                <ChefHat size={11} />From: {item.donorName}
                              </span>
                            )}
                            {/* ✅ FIXED: render string not object */}
                            {ngoName && (
                              <span className="flex items-center gap-1 text-xs text-gray-400">
                                <ShieldCheck size={11} />To: {ngoName}
                              </span>
                            )}
                            <span className="flex items-center gap-1 text-xs text-gray-400">
                              <MapPin size={11} />{item.pickupAddress}
                            </span>
                            <span className="flex items-center gap-1 text-xs text-gray-400">
                              <Clock size={11} />Best before {item.expiryTime}
                            </span>
                            <span className="flex items-center gap-1 text-xs text-gray-400">
                              <Calendar size={11} />{item.createdAt || item.postedAt || "Recently"}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div key={item.id} className="flex items-start gap-4 bg-green-50/50 border border-green-100 rounded-2xl px-5 py-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1">
                          ...
                          {ngoName && (
                            <span className="flex items-center gap-1 text-xs text-gray-400">
                              <ShieldCheck size={11} />Delivered to: {ngoName}
                            </span>
                          )}
                        </div>
                      </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 rounded-2xl px-6 py-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-blue-600 mt-0.5">
              You have delivered <span className="font-bold">{totalDone} orders</span> totalling an estimated{" "}
              <span className="font-bold">{totalDone * 40}+ meals</span> to people in need. 🚛❤️
            </p>
          </div>
        </div>
      </main>
