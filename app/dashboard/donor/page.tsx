  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("http://localhost:8081/api/food-listings", {
        ...
        body: JSON.stringify({ foodName, quantity, pickupAddress, expiryTime, notes, donorId, status: "AVAILABLE" }),
      });
      if (!res.ok) {
      ...
      <div className="relative w-full sm:max-w-lg bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden" style={{ animation: "slideUp 0.35s cubic-bezier(0.16,1,0.3,1)" }}>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-[72vh] overflow-y-auto">
          ...
          <div>
            ...
            <input type="text" value={foodName} onChange={(e) => setFoodName(e.target.value)} placeholder="e.g. Dal Rice, Biryani, Roti Sabzi" className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition" />
          </div>
          <div>
            ...
            <input type="text" value={quantity} onChange={(e) => setQuantity(e.target.value)} placeholder="e.g. 50 meals, 10 kg" className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition" />
          </div>
          <div>
            ...
            <textarea value={pickupAddress} onChange={(e) => setPickupAddress(e.target.value)} placeholder="Full address with landmark" rows={2} className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition resize-none" />
          </div>
          <div>
            ...
            <input type="time" value={expiryTime} onChange={(e) => setExpiryTime(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition" />
          </div>
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="e.g. Contains dairy, packed in boxes..." rows={2} className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition resize-none" />
          </div>
          <button type="submit" disabled={loading || success} className="w-full flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 text-white font-extrabold text-sm py-4 rounded-2xl shadow-lg shadow-orange-200 transition-all">
            {loading ? <><Loader2 size={16} className="animate-spin" />Creating listing...</> : success ? <><CheckCircle2 size={16} />Created!</> : <><Plus size={16} />Post Food Listing</>}
          </button>
        </form>
      </div>
...
export default function DonorDashboard() {
  ...
  const taglines = ["Your generosity turns leftovers into lifelines. 🍱", "One donation feeds a family tonight. Thank you! 🙏", "Every meal shared is a life touched. Keep going! ❤️", "Your kitchen surplus is someone hope. 🌟"];
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        <div className="relative bg-gradient-to-br from-orange-500 via-orange-500 to-orange-600 rounded-3xl px-8 py-7 overflow-hidden shadow-lg shadow-orange-200">
          ...
          <div className="relative flex items-center justify-between gap-6 flex-wrap">
            <div>
              ...
              <p className="text-orange-100 text-sm max-w-sm leading-relaxed">{tagline}</p>
            </div>
            <button onClick={() => setShowModal(true)} className="flex-shrink-0 flex items-center gap-2 bg-white text-orange-600 font-extrabold text-sm px-5 py-3 rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200"><Plus size={18} />Donate Food</button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-7 pt-6 pb-4 border-b border-gray-50 flex items-center justify-between gap-4 flex-wrap">
            <div><h2 className="text-lg font-extrabold text-gray-900">Your Donations</h2><p className="text-sm text-gray-400 mt-0.5">{filteredListings.length} listing{filteredListings.length !== 1 ? "s" : ""} found</p></div>
            <button onClick={() => setShowModal(true)} className="flex items-center gap-1.5 text-xs font-bold text-orange-600 hover:text-orange-700 bg-orange-50 hover:bg-orange-100 border border-orange-200 px-3.5 py-2 rounded-xl transition-colors"><Plus size={14} />New Listing</button>
          </div>
          <div className="px-7 pt-4 flex gap-1.5 flex-wrap">
            {tabs.map(({ key, label, count }) => (
              <button key={key} onClick={() => setActiveTab(key)} className={`flex items-center gap-1.5 text-xs font-bold px-3.5 py-2 rounded-xl transition-all duration-150 ${activeTab === key ? "bg-orange-500 text-white shadow-sm" : "bg-gray-50 text-gray-500 hover:bg-gray-100 border border-gray-100"}`}>
                ...
                return (
                  <div key={listing.id} className="flex items-start gap-4 bg-gray-50 hover:bg-orange-50/50 border border-gray-100 hover:border-orange-100 rounded-2xl px-5 py-4 transition-all duration-150 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {listing.claimedBy && (
                        <div className="mt-2 inline-flex items-center gap-1.5 bg-green-50 border border-green-100 text-green-700 text-xs font-semibold px-2.5 py-1 rounded-lg"><CheckCircle2 size={11} />Claimed by {listing.claimedBy}</div>
                      )}
                    </div>
